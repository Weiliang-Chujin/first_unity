...
/*
 * 商品操作类，生成商品，并初始化其子物体各信息显示
 */
public class CommodityOperation : MonoBehaviour
{
	public GameObject dailyPanel; //每日精选标题容器
    public GameObject commodityPanel; //商品容器
    public PlayerManager playerManager; //玩家管理者类
    ...
    public void Buy(CommodityPrefab commodityObject)
    {
	    ...
	    if (commodityObject.costNum <= playerManager.totalCoin && commodityObject.num >= 0)
	    {
		    //修改总金币显示
		    playerManager.ModifyCoin(-commodityObject.costNum);
